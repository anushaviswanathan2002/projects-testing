...
export function AuthProvider({ children }) {
  const [user, setUser] = useState(() => {
    const saved = localStorage.getItem("memory_current_user");
    return saved ? JSON.parse(saved) : null;
  });

  const getUsers = () => {
    const data = localStorage.getItem("memory_users");
  ...
  const updateStats = (moves, time) => {
    ...
    const updated = users.map((u) => {
      ...
      const stats = {
        ...
        bestMoves:
          u.stats.bestMoves === null ? moves : Math.min(u.stats.bestMoves, moves),
        bestTime:
          u.stats.bestTime === null ? time : Math.min(u.stats.bestTime, time),
      };
      return { ...u, stats };
    });
    saveUsers(updated);
    ...
    setUser(updatedUser);
    localStorage.setItem("memory_current_user", JSON.stringify(updatedUser));
  };
